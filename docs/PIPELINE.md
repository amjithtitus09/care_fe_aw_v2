...
|----------|---------|------|
| `jira-pr-author.md` | `repository_dispatch` (`jira-task`) / `workflow_dispatch` | Pinned-model agent implements a minimal first cut, runs `npm run format` (prettier) so the draft is lint-clean, and opens a **draft** PR labelled `jira-agent`. Does **not** seed QA data. |
...
| `pr-rework.md` | `label_command: state:rework` | Pinned-model fixer. Reads the latest review, QA, **or human** findings, makes a minimal fix, validates (`format`/`lint-fix`/`knip`/`build`/`tsc`), pushes to the branch, and re-labels `state:review`. Always runs `npm run format` for prettier/lint failures — never hand-edits. Hard cap of 3 attempts → `state:human` (a human change request resets the cap). |
